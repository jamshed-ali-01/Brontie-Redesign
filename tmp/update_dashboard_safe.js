...
// 1. Remove Heroicons imports
const heroEnd = lines.findIndex(line => line.includes("@heroicons/react/24/outline"));
let heroStartIdx = -1;
for (let i = heroEnd; i >= 0; i--) {
  if (lines[i].includes('import {')) {
    heroStartIdx = i;
    break;
  }
}
if (heroStartIdx !== -1 && heroEnd !== -1) {
  const newImports = [
  ...
  lines.splice(heroStartIdx, heroEnd - heroStartIdx + 1, ...newImports);
}

// 2. Replace loading and error states using array map.
const loadStart = lines.findIndex(l => l.includes('if (loading) {'));
if (loadStart !== -1) {
  lines.splice(loadStart + 1, 8,
    "    return (",
...
const errStart = lines.findIndex(l => l.includes('if (error) {'));
if (errStart !== -1) {
  lines.splice(errStart + 1, 14,
    "    return (",
    "      <div className=\"min-h-screen bg-[#fcfcfc] flex items-center justify-center\">",
    "        <div className=\"text-center\">",
...
// 4. Find `const handleLogout`
const logoutIdx = lines.findIndex(l => l.includes('const handleLogout = async () => {'));
let endLogoutIdx = -1;
if (logoutIdx !== -1) {
  for (let i = logoutIdx + 1; i < lines.length; i++) {
...
console.log('logout idx', logoutIdx, 'end', endLogoutIdx);

// 5. Replace everything after endLogoutIdx
if (endLogoutIdx !== -1) {
  lines.splice(endLogoutIdx + 1, lines.length - endLogoutIdx - 1);
  const mainReturn = `
  return (
...
`;
  lines.push(...mainReturn.split('\\n'));
} else {
  console.log('Error: Could not find end of handleLogout');
  process.exit(1);
}
