...
// Remove Heroicon imports precisely by looking at first 20 lines
const heroStart = lines.findIndex(l => l.includes('import {') && l.includes('CreditCardIcon'));
const heroEnd = lines.findIndex(l => l.includes("@heroicons/react/24/outline"));
if (heroStart !== -1 && heroEnd !== -1) {
  lines.splice(heroStart, heroEnd - heroStart + 1,
...
// Ensure there's no `loading` syntax err
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('if (loading) {')) {
    // replace lines i+1 to i+8
    lines.splice(i + 1, 8,
      "    return (",
...
// Find `const handleLogout` exactly
...
// Slice lines to keep ONLY up to endLogoutIdx
let newLines = lines.slice(0, endLogoutIdx + 1);
...
newLines.push(...mainReturn.split('\n'));

fs.writeFileSync(filePath, newLines.join('\n'), 'utf-8');
console.log('Final flawless script executed!');
