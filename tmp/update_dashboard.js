...
// 1. Replace imports (Heroicons -> new imports)
...
// 4. Safely replace the MAIN RETURN
const mainReturnTokens = ['return (', '    <div className="min-h-screen bg-gray-50">', '      {/* Header */}'];
const mainReturnRegex = new RegExp(`return \\([\\s\\S]*?<div className="min-h-screen bg-gray-50">[\\s\\S]*?\\{\\/\\* Header \\*\\/\\}`);
const match = content.match(mainReturnRegex);

if (match) {
  const returnStartIndex = match.index;
              ...
              <div className="space-y-6">
                ...
                {selectedVoucher.message && (
                  <div className="bg-[#fff9eb] rounded-xl p-4 border border-[#fde6b3]">
                    ...
                    <p className="text-gray-800 italic relative z-10 before:content-['"'] before:text-4xl before:text-[#f4c24d]/40 before:absolute before:-top-2 before:-left-1 pl-4">&quot;{selectedVoucher.message}&quot;</p>
                  </div>
                )}
              </div>
  ...
  console.log("Could not find Main Return Match");
}
